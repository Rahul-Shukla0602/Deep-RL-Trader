...
class DDPG:
    ...
    def __init__(self, env, time_steps, hidden_dim):
        ...
        self.replay_state = torch.zeros((self.start_size - 1, 3, self.state_dim))
        self.replay_next_state = torch.zeros((self.start_size - 1, 3, self.state_dim))
        self.replay_action = torch.zeros((self.start_size - 1, 1, self.state_dim))
        self.replay_reward = torch.zeros((self.start_size - 1,))
    ...
    def train_on_batch(self):
        ...
        sample = torch.randint(self.time_dim, self.replay_reward.shape[0], [self.batch_size])
